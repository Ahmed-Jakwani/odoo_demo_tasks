...
class HelpdeskTicket(models.Model):
    ...
    sub_category_id = fields.Many2one('helpdesk.subcategory',
                                      string="Sub Category",required=True)
    ...
    description = fields.Html('Description', tracking=True,required=True)
    ...
    def message_new(self, msg_dict, custom_values=None):
        ...
        defaults = {
            ...
            'email_subject': msg_dict.get('new_portal_subject') or _("No Subject"),
    ...
    def create(self, vals):
        ...
        for vals in multi_vals:
            ...
            if not self.env.user.has_group('sh_all_in_one_helpdesk.helpdesk_group_team_leader') or not self.env.user.has_group('sh_all_in_one_helpdesk.helpdesk_group_manager'):
                team = self.env['sh.helpdesk.team'].search([('team_members','in',[self.env.user.id])])  
                if not vals.get('team_id'):
                    ...
                    if self.env.company.sh_display_multi_user:
                        vals.update({
                            'sh_user_ids':[(4, self.env.user.id)]
                        })
            if self.env.user.has_group('sh_all_in_one_helpdesk.helpdesk_group_team_leader') or self.env.user.has_group('sh_all_in_one_helpdesk.helpdesk_group_manager'):
            ...
            if res.team_id and res.team_head and res.user_id and res.sh_user_ids:
                ...
                if res.team_head.partner_id.email_formatted not in email_formatted:
                    email_formatted.append(
                        res.team_head.partner_id.email_formatted)
                if res.user_id.partner_id.email_formatted:
                    if res.user_id.partner_id.email_formatted not in email_formatted:
