...
class PortalHelpdesk(CustomerPortal):
    ...
    def _prepare_home_portal_values(self, counters):
        ...
        if request.env.user.sh_portal_user_access and request.env.user.sh_portal_user_access == 'user':
            ...
            ticket_domain.append(('|'))
            ticket_domain.append(('|'))
            ticket_domain.append(('user_id', 'in', ['project_id.x_allowed_users'])) # Yaminah
            ticket_domain.append(('sh_user_ids', 'in', [request.env.user.id]))
            ticket_domain.append(('user_id', '=', request.env.user.id))
            ticket_domain.append(
                ('partner_id', '=', request.env.user.partner_id.id))
    ...
    def portal_my_tickets(self,
        ...
        if search and search_in:
            domain = AND([domain, ['|',('name', 'ilike', search),(('email_subject', 'ilike', search))]])
        ticket_domain = []
        if request.env.user.sh_portal_user_access and request.env.user.sh_portal_user_access == 'user':
            ticket_domain.append(('|'))
            ticket_domain.append(('|'))
        ...
        elif groupby == 'email_subject':
            grouped_tickets = [
                HelpdeskTicket.concat(*g)
                for k, g in groupbyelem(tickets, itemgetter('email_subject'))
            ]
    ...
    def portal_my_ticket_detail(self,
        ...
        next_url =  request.env['sh.helpdesk.ticket'].sudo().search([('id','>',ticket_id)],order="id",limit=1)
        next_record = next_url.get_portal_url() if next_url else ticket_id
        prev_url =  request.env['sh.helpdesk.ticket'].sudo().search([('id','<',ticket_id)],order="id desc",limit=1)
        previous_record = prev_url.get_portal_url() if prev_url else ticket_id
        values = {
            'token': access_token,
        ...
        if previous_record != ticket_id:
            values.update({'prev_record':previous_record})
        if next_record != ticket_id:
            values.update({'next_record':next_record})
    ...
    def portal_create_ticket(self, **kw):
        try:
            ...
            _logger.info('login user '+ login_user.name)
            if login_user and login_user.login != 'public':
                ...
                if partner_id:
                        ...
                        if login_user.sh_portal_user_access and request.env.user.has_group(
                            ...
                            if request.env.company.sh_default_team_id:
                                ticket_dic.update({
                                    ...
                                    'team_head':
                                    request.env.company.sudo().sh_default_team_id.
                                    team_head.id,
                                    'user_id':
                    ...
                    if kw.get('new_portal_subject'):
                        ticket_dic.update({
                            'email_subject':
                            str(kw.get('new_portal_subject')),
                        })
                    ...
                    if kw.get('portal_project'
                              ) and kw.get('portal_project') != 'project':
                        # Yaminah
                        project = request.env['project.project'].sudo().search(
                        [('id', '=', int(kw.get('portal_project')))], limit=1)
                        if project.functional_lead:
                            ticket_dic.update({
                                'user_id': project.functional_lead.id,
                            })   
                        # Yaminah 
                        ticket_dic.update({
                            'project_id':
                            int(kw.get('portal_project')),
                        })
